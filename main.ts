...
namespace Tinybit {
    ...
    let car_flag_old = 0;		//0:両方のモーターが正転 1:両方のモーターが逆転 2:左回転 3:右回転
    let car_flag_new = 0;		//0:両方のモーターが正転 1:両方のモーターが逆転 2:左回転 3:右回転
    function setPwmMotor(mode: number, speed1: number, speed2: number): void {
        ...
        if(car_flag_new != car_flag_old)			//前回は前進でしたが、今回は後退です
        {
    ...
    export function car_sport(sp_L:number,sp_R:number)
    {
        ...
        else if(sp_L>0 && sp_R<0)	//左旋回
        {
            car_flag_new = 2;
        }
        else if(sp_L<0 && sp_R>0)	//右旋回
        {
            car_flag_new = 3;
        }

        if(car_flag_new != car_flag_old)		//状態の変更あり
        {
            let bufff = pins.createBuffer(5);
            bufff[0] = MOTOR;
            bufff[1] = 0; bufff[2] = 0; bufff[3] = 0; bufff[4] = 0;
